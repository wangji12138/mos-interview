...
public class SkuReadServiceImpl implements SkuReadService {
     ...
     * 实体字段数组
     */
    private final static String[] FILED = {"id", "name", "artNo", "spuId", "skuType", "price", "inventoryList"};
    ...
    private static SkuDO toSkuDO(String str) {
        ...
        if (strings.length != FILED.length) {
            throw new RuntimeException("当前数据结构异常");
        }
        Map<String, Object> map = new HashMap<>(FILED.length);
        for (int i = 0; i < FILED.length - 1; i++) {
            map.put(FILED[i], strings[i]);
        }
        map.put(FILED[FILED.length - 1], JSON.parseArray(strings[FILED.length - 1], ChannelInventoryDO.class));
        //转换为实体
