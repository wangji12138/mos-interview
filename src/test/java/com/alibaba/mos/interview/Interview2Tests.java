...
class Interview2Tests {
    ...
    void statisticsDataTest() {
        //定义总sku价格容器
        Map<Integer, Set<String>> allPriceSkuIdMap = new LinkedHashMap<>(10000);
        //定义sku种数
        ...
        List<Map.Entry<Integer, Set<String>>> mapList = new ArrayList<>(allPriceSkuIdMap.entrySet());
        //排序
        ...
        for (int i = 0, skuIndex = 0; i < mapList.size(); i++) {
            Map.Entry<Integer, Set<String>> entry = mapList.get(i);
            ...
            if (null == medianSkuStr && skuIndex >= skuMedianIndex[0]) {
                if (skuMedianIndex.length == 2 && skuIndex < skuMedianIndex[1]) {
                    String one = entry.getValue().iterator().next();
                    String two = mapList.get(i + 1).getValue().iterator().next();
     ...
     * @return
     */
    private int appendAllPriceSkuIdMap(String skuId, int skuPrice, Map<Integer, Set<String>> allPriceSkuIdMap) {
        if (null == allPriceSkuIdMap) {
            throw new RuntimeException("所有价格map未初始化");
        }
        //判断map中是否包含 当前价格
        //包含则累计
        Set<String> skuIdSet = allPriceSkuIdMap.get(skuPrice);
        if (null != skuIdSet) {
            //有序添加
            allPriceSkuIdMap.get(skuPrice).add(skuId);
            return 0;
        }
        //不包含，则新增
        else {
            skuIdSet = new HashSet<>(128);
            skuIdSet.add(skuId);
            allPriceSkuIdMap.put(skuPrice, skuIdSet);
            return 1;
        }
    }
